...
export const computedCalendarData = (dataList: CalendarData[]): DataMap => {
    ...
    for (const event of dataListWithMetadata) {
        for (const date of event.metadata.datesInRange) {
            ...
            dataMap[date] = dataMap[date] ? [...dataMap[date], eventCopy] : [eventCopy];
        }
    }
    ...
    Object.keys(dataMap)
        ...
        .forEach((eventDate) => {
            ...
            const prevEventList = updateStartEndFlags(dataMap[prevDate] || [], prevDate);
            const currentEventList = updateStartEndFlags(dataMap[eventDate] || [], eventDate);
...
const updateStartEndFlags = (dataListWithMetadata: CalendarDataWithMetadata[], referenceDate: string): CalendarDataWithMetadata[] => {
    return dataListWithMetadata.map((dataWithMetadata) => {
        if (!dataWithMetadata) return null;
        const dateIndex = dataWithMetadata.metadata.datesInRange?.findIndex((date) => date === referenceDate);
