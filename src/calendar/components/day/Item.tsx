...
export const Item = ({ date, isDisabled, item, weekStartDay, theme, index, maxVisibleCount }: IProps) => {
    ...
    const dayItemStyle: StyleProp<ViewStyle>[] = [styleSheet.dayItem, { height: `${Math.floor(100 / (maxVisibleCount + 2))}%` }];

    if (item === null) {
        if (theme.dayItemStyle) dayItemStyle.push(theme.dayItemStyle);
    ...
    if (datesInRange.length) {
        ...
        if (daysDifference < daysRemaining) scheduleDaysRemaining = daysDifference + 1;
    }
    ...
    const textStyle: StyleProp<TextStyle> = { fontSize: 48 / weeks, color: 'white', left: isShowContents ? 2 : 0, width: `${width}%` };
