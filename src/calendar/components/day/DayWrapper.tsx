...
import { CalendarData, CalendarDataWithMetadata, OnDatePress } from '../../type';
import { Day } from './Day';

const handleItems = (items: CalendarDataWithMetadata[]): CalendarData[] => {
    return items.map((v) => {
        if (!v) return null;
        const { metadata, ...data } = v;
        return data;
    });
};
...
export const DayWrapper = React.memo(({ date, isDisabled }: IProps) => {
    ...
    const { selectedDate, onDatePress, renderDate, weekStartDay, dataMap, theme, renderMoreItemText, maxVisibleCount } = useContext(CalendarContext);
    ...
    const onDateSelectCb: OnDatePress = useCallback((date, options) => {
        return onDateSelectRef.current?.(date, options);
    }, []);

    const items: CalendarDataWithMetadata[] = dataMap[format(date, 'yyyy-MM-dd')] || [];

    if (renderDate) {
        return renderDate({ date, isDisabled, isSelected, isToday, itemsWithMetadata: items, items: handleItems(items) });
    }
    const onPress = () => {
        onDateSelectCb(date, {
            isSelected,
...
const styleSheet = StyleSheet.create({
    flex: { flex: 1 },
    coverContainer: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 101 },
});
