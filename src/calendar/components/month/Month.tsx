...
export const Month = React.memo(({ index }: { index: number }) => {
    const { initialDateRef, weekStartDay, height, width } = useContext(CalendarContext);
    ...
    if (height) monthStyle.push({ height });
    if (width) monthStyle.push({ width });
    ...
    return (
        <View style={monthStyle}>
            ...
            {weeks.map((week) => {
                return <Week key={week[0]?.toISOString()} days={week} firstDayOfMonth={firstDayOfMonth} />;
            })}
        </View>
    );
});
