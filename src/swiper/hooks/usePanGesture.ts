...
import { Gesture, GestureStateChangeEvent, GestureTouchEvent, GestureUpdateEvent, PanGestureHandlerEventPayload } from 'react-native-gesture-handler';
...
export const usePanGesture = ({ curIndex, translate, viewSize }: Iprops) => {
    ...
    const isMinIndex = useDerivedValue(() => curIndex <= minIndex, [curIndex, minIndex]);
    const isMaxIndex = useDerivedValue(() => curIndex >= maxIndex, [curIndex, maxIndex]);

    const initTouchX = useSharedValue(0);
    const initTouchY = useSharedValue(0);
    const startTranslate = useSharedValue(0);

    const handleBegin = (e: GestureStateChangeEvent<PanGestureHandlerEventPayload>) => {
        'worklet';
    ...
    const handleTouchesMove = ({ changedTouches }: GestureTouchEvent, mgr: GestureStateManagerType) => {
        'worklet';
        ...
        if ((isMinIndex.value && translateDelta > 0) || (isMaxIndex.value && translateDelta < 0)) {
            mgr.fail();
        }
    };

    const handleUpdate = ({ translationX, translationY }: GestureUpdateEvent<PanGestureHandlerEventPayload>) => {
        'worklet';
        ...
        if (Math.abs(crossAxisTranslation) > 10 && Math.abs(crossAxisTranslation) > Math.abs(translation)) return;
        ...
        if (page >= minIndexAnim.value && page <= maxIndexAnim.value) {
            ...
            const boundary = page < minIndexAnim.value ? minIndexAnim.value : maxIndexAnim.value;
            const overflowPercentage = boundary - page;
            const clampedTranslation = updatedTranslate - overflowPercentage * viewSize.value;
            const bounceTranslation = overflowPercentage * viewSize.value;
            translate.value = clampedTranslation + bounceTranslation;
        }
    };

    const handleEnd = ({ velocityX, velocityY, translationX, translationY }: GestureStateChangeEvent<PanGestureHandlerEventPayload>) => {
        'worklet';
        ...
        if (Math.abs(crossAxisTranslation) > Math.abs(translation)) return;

        // Determine if it's a fling gesture
        const isFling = Math.abs(velocity) > 500;
        let velocityOffset = isFling ? viewSize.value / 2 : 0;
        if (velocity < 0) velocityOffset *= -1;

        let targetPage = initialIndex - Math.round((translate.value + velocityOffset) / viewSize.value);
        targetPage = Math.max(minIndexAnim.value, Math.min(maxIndexAnim.value, targetPage));

        translate.value = withTiming(-(targetPage - initialIndex) * viewSize.value, {
            duration: 500,
    ...
    const panGesture = useMemo(() => {
        return Gesture.Pan().onBegin(handleBegin).onTouchesMove(handleTouchesMove).onUpdate(handleUpdate).onEnd(handleEnd);
    }, [initTouchX, initTouchY, initialIndex, isMaxIndex, isMinIndex, maxIndexAnim, minIndexAnim, startTranslate, translate, vertical, viewSize]);
